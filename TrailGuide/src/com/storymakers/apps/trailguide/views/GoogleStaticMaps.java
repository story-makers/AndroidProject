...
/**
 * <p>
 * Constructs URLs that load static images of a given location. To use, add your
 * Google Static Maps API key (this is <em>not</em> a normal Google Maps API
 * key) to your Android manifest in the &lt;application /&gt; section:
 * </p>
 * 
 * <pre>
 *         &lt;meta-data
...
public class GoogleStaticMaps {
	...
	public static final String METADATA_KEY_API_KEY = "com.google.android.maps.v2.API_KEY";

	private final static Uri BASE_URL = Uri
	...
	/**
	 * Gets a map URL based on the parameters passed into the constructor and
	 * the parameters passed in here. The "scale" parameter is automatically set
	 * based on the DPI {@link DisplayMetrics#scaledDensity} from the provided
	 * context.
	 * 
	 * @param latitude
	 * @param longitude
	 ...
	 *            of the image, in pixels
	 * @param sensor
	 *            if the result came from a GPS sensor or similar, this should
	 *            be set to true
	 * @param marker
	 *            marker style information or null for no marker. Position will
	 *            be the same as the center.
	 * @return
	 */
	public Uri getMap(float latitude, float longitude, int width, int height,
			boolean sensor, String marker) {
		final Builder b = BASE_URL.buildUpon();
		b.appendQueryParameter(PARAMETER_KEY, mKey);
		b.appendQueryParameter(PARAMETER_SIZE, width / mScale + "x" + height
				/ mScale);
